...
public:
	...
	bool isChunkInSphere(vec3i chunk, vec3f center, float radius)
	{
		...
		for(int x = -1; x<=1; x+=2)
		{
			for(int y = -1; y<=1; y+=2)
			{
				for(int z = -1; z<=1; z+=2)
				{
					//计算每个格子的真实坐标, chunk的位置 + 每个小格子的offset
	...
	HRESULT DumpVoxelHash(ID3D11DeviceContext* context, const std::string &filename, DX11SceneRepHashSDF& hash, const vec3f& camPos, float radius, float dumpRadius = 0.0f, vec3f dumpCenter = vec3f(0.0f,0.0f,0.0f)) {
		...
		//write binary
		std::ofstream sdf_out_binary;
		sdf_out_binary.open("./DataDump/sdf_binary.ply", std::ios::binary);
						...
						if(m_grid[index] != NULL && m_grid[index]->isStreamedOut()) // As been allocated and has streamed out blocks
						{
							...
							for (size_t i = 0; i < chunkDesc->m_ChunkDesc.size(); i++) {
								...
								for (unsigned int j = 0; j < SDF_BLOCK_SIZE * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE; j++) {
									...
									sdf_out_ply <<voxelWordPos.x <<" " <<voxelWordPos.y <<" " <<voxelWordPos.z 
									...
									//3. in own binary
									sdf_out_binary.write((const char*)&voxelWordPos.x, sizeof(voxelWordPos.x));
									sdf_out_binary.write((const char*)&voxelWordPos.y, sizeof(voxelWordPos.y));
									sdf_out_binary.write((const char*)&voxelWordPos.z, sizeof(voxelWordPos.z));
									sdf_out_binary.write((const char*)&sdf, sizeof(sdf));
