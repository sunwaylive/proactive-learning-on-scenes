...
void ITMSceneReconstructionEngine_CPU<TVoxel,ITMVoxelBlockHash>::IntegrateIntoScene(ITMScene<TVoxel,ITMVoxelBlockHash> *scene, const ITMView *view, const ITMPose *pose_d)
{
	...
	Matrix4f M_d, M_rgb;//深度和rgb矩阵
	Vector4f projParams_d, projParams_rgb;//投影参数
	...
	ITMHashEntry *hashTable = scene->index.GetEntries();//所有entry都在这个里面
	int *liveEntryIDs = scene->index.GetLiveEntryIDs();//取出有效的entryID

	for (int entryId = 0; entryId < scene->index.noLiveEntries; entryId++)
	{
		Vector3i globalPos;
		const ITMHashEntry &currentHashEntry = hashTable[liveEntryIDs[entryId]];//取出hashtable中有效的entry，这些liveEntry信息放在liveEntryIDs中
		...
		for (int z = 0; z < SDF_BLOCK_SIZE; z++) for (int y = 0; y < SDF_BLOCK_SIZE; y++) for (int x = 0; x < SDF_BLOCK_SIZE; x++)
		{
			...
			locId = x + y * SDF_BLOCK_SIZE + z * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE;//1维的位置ID
			//pt_model可能就是存放的最终的3D world model
