...
void GCoptimization::specializeDataCostFunctor(UserFunctor f) {
...
template <typename UserFunctor>
void GCoptimization::specializeSmoothCostFunctor(UserFunctor f) {
...
void GCoptimization::setLabelOrder(LabelID* order, LabelID size)
{
...
void GCoptimization::handleError(char *message)
{
...
void GCoptimization::printStatus1(char* extraMsg)
{
	if ( m_verbosity < 1 )
		return;
	if ( extraMsg )
...
GCoptimization::DataCostFnSparse::DataCostFnSparse(DataCostFnSparse& src)
...
void GCoptimization::DataCostFnSparse::set(LabelID l, SparseDataCost* costs, SiteID count)
{
...
GCoptimization::EnergyTermType GCoptimization::DataCostFnSparse::search(DataCostBucket& b, SiteID s)
{
	...
	//
	SparseDataCost* L = b.begin;
	SparseDataCost* R = b.end-1;
	if ( R - L == m_num_sites )
		return b.begin[s].cost; // special case: this particular label is actually dense
	do {
		SparseDataCost* mid = (SparseDataCost*)((((size_t)L+(size_t)R) >> 1) & cDataCostPtrMask);
		if (s < mid->site)
...
GCoptimization::SiteID GCoptimization::DataCostFnSparse::queryActiveSitesExpansion(LabelID alpha_label, LabelID* labeling, SiteID* activeSites)
{
	SparseDataCost* next = m_buckets[alpha_label*m_buckets_per_label].begin;
	SparseDataCost* end  = m_buckets[alpha_label*m_buckets_per_label + m_buckets_per_label-1].end;
	SiteID count = 0;
	for (; next < end; ++next) {
