...
void NBV::setInput(DataMgr *pData)
{
  ...
  x_sdf_slice_plane = pData->getXSlicePlane();
  y_sdf_slice_plane = pData->getYSlicePlane();
  z_sdf_slice_plane = pData->getZSlicePlane();
}
...
void NBV::runSDFSlice()
{
  ...
  double half_sdf_cell_length = 0.004 / global_paraMgr.data.getDouble("Max Normalize Length") / 1.0f;//half grid length

  cout<<"half grid length: " <<half_sdf_cell_length <<endl;
  if (global_paraMgr.nbv.getBool("Show SDF Slice X"))
  {
    double x_pos = global_paraMgr.poisson.getDouble("Current X Slice Position");
    cout<<x_pos<<endl;

    GlobalFun::computePointsOnPlane(sdf_voxels, half_sdf_cell_length, 'X', x_pos, (*sdf_slices)[0]);
    cout<<"sdf x slice node num: " <<(*sdf_slices)[0]->vert.size() <<endl;

    //update colors in slice planes
    GlobalFun::computeAnnNeigbhors((*sdf_slices)[0]->vert, x_sdf_slice_plane->vert, 1, false, "runSDFSlice");
    for (int i = 0; i < x_sdf_slice_plane->vert.size(); ++i)
    {
      CVertex &v = x_sdf_slice_plane->vert[i];
      v.P()[0] = x_pos;//move slice plane to the right position
      if (!v.neighbors.empty())
      {
        CVertex &nearest = (*sdf_slices)[0]->vert[v.neighbors[0]];
        double dist = GlobalFun::computeEulerDist(v.P(), nearest.P());
        if (dist < half_sdf_cell_length){
          v.eigen_confidence = nearest.eigen_confidence;
        }
      }
    }
  }
  ...
  if(global_paraMgr.nbv.getBool("Show SDF Slice Y"))
  {
    ...
    GlobalFun::computePointsOnPlane(sdf_voxels, half_sdf_cell_length, 'Y', y_pos, (*sdf_slices)[1]);
    cout<<"sdf y slice node num: " <<(*sdf_slices)[1]->vert.size() <<endl;

    //update colors in slice planes
    GlobalFun::computeAnnNeigbhors((*sdf_slices)[1]->vert, y_sdf_slice_plane->vert, 1, false, "runSDFSlice");
    for (int i = 0; i < y_sdf_slice_plane->vert.size(); ++i)
    {
      CVertex &v = y_sdf_slice_plane->vert[i];
      v.P()[1] = y_pos;//move slice plane to the right position
      if (!v.neighbors.empty())
      {
        CVertex &nearest = (*sdf_slices)[1]->vert[v.neighbors[0]];
        double dist = GlobalFun::computeEulerDist(v.P(), nearest.P());
        if (dist < half_sdf_cell_length){
          v.eigen_confidence = nearest.eigen_confidence;
        }
  ...
  if(global_paraMgr.nbv.getBool("Show SDF Slice Z"))
  {
    ...
    GlobalFun::computePointsOnPlane(sdf_voxels, half_sdf_cell_length, 'Z', z_pos, (*sdf_slices)[2]);
    cout<<"sdf x slice node num: " <<(*sdf_slices)[2]->vert.size() <<endl;

    //update colors in slice planes
    GlobalFun::computeAnnNeigbhors((*sdf_slices)[2]->vert, z_sdf_slice_plane->vert, 1, false, "runSDFSlice");
    for (int i = 0; i < z_sdf_slice_plane->vert.size(); ++i)
    {
      CVertex &v = z_sdf_slice_plane->vert[i];
      v.P()[2] = z_pos;//move slice plane to the right position
      if (!v.neighbors.empty())
      {
        CVertex &nearest = (*sdf_slices)[2]->vert[v.neighbors[0]];
        double dist = GlobalFun::computeEulerDist(v.P(), nearest.P());
        if (dist < half_sdf_cell_length){
          v.eigen_confidence = nearest.eigen_confidence;
        }
